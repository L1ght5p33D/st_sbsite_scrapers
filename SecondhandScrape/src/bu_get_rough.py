...
def scrape_url_ret_obj(url, driver):
	...
	if "sellertype:<strong>" in t.replace(" ","").lower():
		s1 = t.replace(" ","").lower().split("sellertype:<strong>")[1].split("</strong>")[0]
		print("seller type get :: " + str(s1))

		bod["p_seller"] = s1

	if "model:<strong>" in t.replace(" ","").lower():
		s1 = t.replace(" ","").lower().split("model:<strong>")[1].split("</strong>")[0]
		print("model field get :: " + str(s1))

		bod["p_model"] = s1

	if "shipping:<strong>" in t.replace(" ","").lower():
		s1 = t.replace(" ","").lower().split("shipping:<strong>")[1].split("</strong>")[0]
		print("shipping type get :: " + str(s1))

		bod["p_shipping"] = s1

	if "posted:<strong>" in t.replace(" ","").lower():
		s1 = t.replace(" ","").lower().split("posted:<strong>")[1].split("</strong>")[0]
...
# 	]
	...
	try:
		...
		stf = 0
		try:
			stf = float(dimmap['widthFracNumer']) // float(dimmap["widthFracDenom"])
	...
	try:
		...
		stf = 0
		try:
			stf = float(dimmap['thicknessFracNumer']) // float(dimmap["thicknessFracDenom"])
		except Exception as e:
			print("no width inches")
...
# Good on everything
	...
	if "Location: <strong>" in t:
		...
		if c_split not in g_loc_map.keys():
			geocoder = OpenCageGeocode( os.environ['GOOGLE_API_KEY_STSCRAPE_GEOCODE'] )
			query = c_split
			results = geocoder.geocode(query)
			if len(results)> 0:
				try:
					g_loc_map[c_split] = (results[0]["geometry"]["lat"], results[0]["geometry"]["lng"])
					print("geocode results")
					pprint(results)
					print("SET GEOCODE COORDS ::: " + str(results[0]["geometry"]["lat"]) + " __ " + str(results[0]["geometry"]["lng"]))
				...
				print("Using cached g loc map location COORDS" + str(g_loc_map[c_split][0]) + " __ " + str(g_loc_map[c_split][1]))
	...
	for imreg in img_split:
		...
		if "Length:<strong>" in imreg_check.replace(" ",""):
	...
	for bs in board_slugs_get:
		...
		get_cdn_imgs.append("https://www.secondhandboards.com/boardimages" + bss)

				#Think deprecated
	# bod["cdnImageList"] = get_cdn_imgs

...
data_dir=/home/si/os_projects/SurfTrader/1_serve_surftrader/SCRAPE/Surftrade_Scrapers/auto_scrape_scripts/IconsOfSurf/data
...
if [ -f "data/cur_urls" ]; then
	mv data/cur_urls ./archive/cur_url_${time_string}
fi
...
# Gets board info page urls from list pages
    python src/run_gen_diff_urls.py $2 $data_dir $webdriver_path

    echo " Call run_get_updates.py ............"
# run_get_updates.py calls product_page_url_scrape.py and creates scraped_items file
    python src/run_get_updates.py $2 $data_dir $webdriver_path
    ...
    if [[ $1 == "Index" ]]; then
      python src/run_index_preElastic.py $2 $data_dir
    fi

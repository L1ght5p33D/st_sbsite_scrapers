...
def scrape_product_url(productUrl, baseurl, data_dir, webdriver_url):
    #gsel = webdriver.PhantomJS()
    #gsel = webdriver.Chrome('chromedriver')
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    gsel = webdriver.Chrome(webdriver_url, options=chrome_options)
    ...
    if "longboard" in baseurl:
        ...
        boardType = "Funboard"
    if "fish" in productUrl:
        boardType = "Fish"
    try:
        imgMainSplit = bPageSrc.split('class="lightbox"')
        imgMainSplitHref = imgMainSplit[1].split("src=\"")
    ...
    boardTitle = ""
    try:
        boardTitleSplit = bPageSrc.split("class=\"product_name\">")
        boardTitle = boardTitleSplit[1].split("</h1>")[0]
        print("board title::" + boardTitle)
    except:
        print("couldnt find board title")
    
    boardPriceNum = 0
    try:
        boardPriceSplit = bPageSrc.split('money">')[1].split("<")[0].replace("$","")
        
        boardPriceNum = round(float(boardPriceSplit), 2)
        print("board price ::::" + str(boardPriceNum))
    except:
        print("couldnt find board price")
    
    descGet = ""

    try:
        descSplit = bPageSrc.split('class="description"')[1]
    ...
    boardDesc_rebuild = ""
    boardDescPiecesList = descSanitize.split(">")
    for piece in boardDescPiecesList:
        boardDesc_rebuild += piece.split("<")[0]

    boardDesc = boardDesc_rebuild
    ...
    boardDataObj["itemUUID"] = boardUUID
    ...
    if boardDataObj["title"] != "":
        with open(data_dir + "/scraped_items", "a+") as fSave:
